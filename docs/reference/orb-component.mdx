...
```ts
type OrbTheme = 'debug' | 'circle' | 'bars' | 'cloud' | 'radial' | 'ocean'
```
...
- `ocean`: a glass sphere with a calm sea inside. The waterline breathes while idle, ripples with input, swells and foams with output, and the light inside pulses while thinking. Ships light and dark presets (`scheme`) and takes hex overrides through `palette`.
...
The ocean theme follows the surrounding surface through `scheme`, and any of its six colors can be
replaced with a hex value:
...
<Orb
  ...
  theme="ocean"
  scheme="dark"
  palette={{ glow: '#ffd9a0', deep: '#03101f' }}
  interactive={false}
/>
```
